...
> Current release note: v0.4.2 keeps the active plugin route image-first only, preserves the native-PPTX firewall for ambiguous "make a PPT" requests, treats explicit Deckit invocations such as `@deckit 介绍一下 F-16 战斗机` as deck-production requests by default, adds PPTX auditing to reject substitute visual native-PPTX decks, and restores the public marketplace display name to "Deckit". Historical alternate-output research remains in archived docs, but those routes are no longer offered by the plugin skills or dev CLI.
...
- Plugin version: **0.4.2** (was 0.1.0 at the start of the roadmap; v0.4.x keeps image-first generation the only active path).
...
- Both `deckit-dev inspect` and `inspect-marketplace` pass with the v0.4.2 manifest.
